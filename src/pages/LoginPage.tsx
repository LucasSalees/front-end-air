...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
      ...
      try {
          ...
          const { token, primeiroAcesso } = response.data; // O Java deve retornar isso

          localStorage.setItem('token', token);

          if (primeiroAcesso) {
              // Se for o primeiro acesso, manda para a tela de troca
              navigate('/reset-password');
          } else {
              // Se não, vai direto para o dashboard
              navigate('/dashboard');
          }
      } catch (err) {
          alert("Erro no login");
